 ...
 * Restored functionality with Cape Trimming and Error Safety.
 */
...
const CONFIG = {
    ...
    // DEBUG: Keeps Red Lines visible so we can verify the cut.
    // Set to FALSE once you confirm the cut is good.
    DEBUG: true,

    VIEWPORT: {
        ZOOM_LEVEL: 1.8,  // Safe zoom (shows head and shoulders)
        TOP_OFFSET: 0.05, // 5% from top
    },
    ...
    PHYSICS: {
        GRAVITY_FACE: 0.05,
        GRAVITY_AIR: 0.5,
        TERMINAL_VEL_FACE: 2.0,
        FRICTION_BOUNDARY: 0.35,
        WOBBLE_SPEED: 0.03,
        WOBBLE_AMP: 3,
    },
...
export class MirrorShadow {
    ...
    async init() {
        try {
            console.log("Mirror: Loading...");
            const raw = await this.loadImage(CONFIG.ASSETS.URL);

            console.log("Mirror: Processing Silhouette...");
            ...
            // Force loop anyway to show debug grid if needed
            this.loop();
        }
    }
    ...
    processSilhouette(sourceImg) {
        return new Promise((resolve) => {
            ...
            for (let i = 0; i < data.length; i += 4) {
                ...
                if (diff < CONFIG.ASSETS.CHROMA_TOLERANCE) {
                    data[i + 3] = 0;
                } else {
    ...
    resize() {
        ...
        if (this.image) {
            const scale = (this.canvas.width / this.image.width) * CONFIG.VIEWPORT.ZOOM_LEVEL;
            this.layout = {
                scale: scale,
    ...
    spawnTear(char) {
        ...
        this.particles.push({
            char: char, x: sx, y: sy + 10, originX: sx,
            vx: 0, vy: 0, life: 1.0, angle: (Math.random() - 0.5) * 0.2, onFace: true
        });
    }
    ...
    trimCape(isDebug) {
        ...
        // ADJUST THESE TO TUNE THE CUT
        const neckY = l.y + (l.height * 0.22); // Neck Level
        const shoulderW = l.width * 0.16;      // Shoulder Width
        ...
        this.ctx.bezierCurveTo(
            l.x + l.width * 0.1, neckY + l.height * 0.05,
            cx - shoulderW, neckY + l.height * 0.05,
            cx - shoulderW * 0.9, l.height
        );
        ...
        this.ctx.bezierCurveTo(
            l.x + l.width * 0.9, neckY + l.height * 0.05,
            cx + shoulderW, neckY + l.height * 0.05,
            cx + shoulderW * 0.9, l.height
        );
...
export function initMirrorShadow() { new MirrorShadow(); }
